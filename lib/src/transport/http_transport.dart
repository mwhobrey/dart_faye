...
class HttpTransport extends BaseTransport {
  ...
  Future<void> connect(String url, {Map<String, String>? headers}) async {
    ...
    try {
      ...
      if (responseMessage['successful'] == true) {
        ...
        _logger.severe('HTTP: Handshake failed: ${responseMessage['error']}');
  ...
  Future<dynamic> _sendRequest(dynamic data) async {
    ...
    if (_connectionType == 'callback-polling' && _jsonpCallback != null) {
      ...
      final callbackParam =
          '$_jsonpCallback=${DateTime.now().millisecondsSinceEpoch}';
  ...
  void _startPolling() {
    ...
    _polling = true;
    _pollingTimer =
        Timer.periodic(Duration(milliseconds: _pollingInterval), (timer) {
      _poll();
    });
  }
